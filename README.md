...
```typescript
import { GamingCanvas, GamingCanvasResolutionScaleType, GamingCanvasSetSize } from '@tknight-dev/gaming-canvas';
...
const canvases: HTMLCanvasElement[] = GamingCanvas.initialize(container, {
    resolutionScaleType: GamingCanvasResolutionScaleType.PIXELATED, // PIXELATED for chunky 8bit styled upscaling of fixed resolution to fill the container element
    resolutionWidthPx: 640, // Default aspect ratio is 16/9 so the final fixed dimensions are 640x320 (like Youtube's 320p) and will never change
});
...
| canvasCount?                  | number                          | 1           | How many canvas elements (layers) to be generated (canvas1: id=`gaming-canvas-canvas1`, zIndex=10; canvas2: id=`gaming-canvas-canvas2`, zIndex=20, ... )                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| elementInjectAsCanvas?        | HTMLElement[]                   | []          | Elements are injected into the canvas container element (id=`gaming-canvas-container-canvas`) which is subject to scaling. Use CSS position `absolute` to fit your pseudo-canvas(es) relative to the canvas container                                                                                                                                                                                                                                                                                                                                                     |
| elementInjectAsOverlay?       | HTMLElement[]                   | []          | Elements are injected into the overlay container element (id=`gaming-canvas-container-overlay`) container which is sized to match the scaled size of the canvas(es) and is not subject to scaling. Use CSS position `absolute` to fit your overlay(s) relative to the overlay container                                                                                                                                                                                                                                                                                   |
| elementInteractive?           | HTMLElement                     | canvas      | The input listeners are bound to this element, but `GamingCanvasInputPosition` is always relative to the canvas. This element should be equal to or larger then the canvas in all dimensions. EG the canvas is zIndex=20, an overlay(s) is zIndex=21, so the interactive elment should be a super overlay at zIndex=30 as the overlay(s) at zIndex=21 would intercept input liseners at zIndex=20 unless `pointer-events` and `touch-events` are set to `none` on the CSS for the overlay(s). Note: the canvas with the highest zIndex is the default elementInteractive. |
...
| orientationLeftOnPortait?     | boolean                         | false       | Rotate left instead of right in portrait mode                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| resolutionScaleToFit?         | boolean                         | true        | Enables the scalling feature of `resolutionByHeightPx` when using a non-null value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| resolutionScaleType?          | GamingCanvasResolutionScaleType | `ANTIALIAS` | Set the scaling style(s) of canvas(es) to the container element's dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| resolutionWidthPx?            | number or `null`                | `null`      | `null` sets the canvas dimensions to match the container element, and not `null` sets a fixed resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
## Models: GamingCanvasResolutionScaleType (enum)

| Key       | Description         |
| --------- | ------------------- |
| ANTIALIAS | Smoothed out pixels |
| PIXELATED | Chunky 8bit style   |
...
setTimeout(() => {
    ...
    clearInputQueue(); // Clear any inputs still in the queue to be processed. Watch out for gamepad/keyboard/mouse/touch inputs that never registered a buttonUp event.
}, 1000);
```
...
// Vibrate with two pulses
if (GamingCanvas.isVibrateSupported()) {
    GamingCanvas.vibrate([200, 100, 300]); // 200ms vibrate, 100ms nothing, 300ms vibrate
}

// Vibrate with two pulses, repeating
if (GamingCanvas.isVibrateSupported()) {
    GamingCanvas.vibrate([200, 100, 300], true, 100); // same pattern as above, but repeat after waiting 100ms for even dead spaces between pulses
    setTimeout(() => {
        GamingCanvas.vibrateCancel();
    }, 3000); // 3 seconds
}
```
...
- Add automatic and/or manual mouse fader?
- Add support for gamepad haptics (once Chrome and Firefox figure out how they want to implement it [MDN](https://developer.mozilla.org/en-US/docs/Web/API/GamepadHapticActuator))
