...
export class GamingCanvas {
	...
	public static initialize(elementParent: HTMLElement = document.body, options: GamingCanvasOptions = {}): HTMLCanvasElement[] {
		...
		options.inputKeyboardEnable && GamingCanvasEngineKeyboard.initialize(GamingCanvas.inputQueue, options.inputKeyboardPreventTab);
	 ...
	 *
	 * @param mobile? true detects mobile
	 * @param tablet? true detects tablets
	 */
	public static detectDevice(mobile: boolean = true, tablets: boolean = false): boolean {
		const id: string = navigator.userAgent || (<any>window).vendor || (<any>window).opera;
		...
		} else if (tablets === true) {
			return /android|ipad|playbook|silk/i.test(id);
		}
	}
