...
export class GamingCanvas {
	...
	/**
	 * detectmobilebrowsers.com | unlicense | 'This is free and unencumbered software released into the public domain.' [10/08/25]
	 *
	 * @param type defaults to mobile and tablets
	 */
	public static detectDevice(
		types: GamingCanvasOptionsDetectDeviceType[] = [GamingCanvasOptionsDetectDeviceType.MOBILE, GamingCanvasOptionsDetectDeviceType.TABLET],
	): boolean {
		let id: string = navigator.userAgent || (<any>window).vendor || (<any>window).opera,
			type: GamingCanvasOptionsDetectDeviceType;

		for (type of types) {
			if (type === GamingCanvasOptionsDetectDeviceType.TABLET) {
				if (/android|ipad|playbook|silk/i.test(id) === true) {
					return true;
				}
			} else {
				const m: boolean =
				...
				if (type === GamingCanvasOptionsDetectDeviceType.MOBILE) {
					if (m === true) {
						return true;
					}
				} else {
